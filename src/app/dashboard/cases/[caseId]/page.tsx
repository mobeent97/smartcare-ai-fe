            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-2.5">
                <button
                  type="button"
                  onClick={async () => {
                    const url = api.getSessionPdfUrl(caseId);
                    console.log('[handoff-pdf] downloading', url);
                    try {
                      const res = await fetch(url, { method: 'GET', credentials: 'omit' });
                      console.log('[handoff-pdf] response', res.status, res.headers.get('content-type'));
                      if (!res.ok) {
                      ...
                      a.href = objUrl;
                      a.download = `triage-${caseId.slice(0, 8)}.pdf`;
                      document.body.appendChild(a);
                      a.click();
                      document.body.removeChild(a);
                      setTimeout(() => URL.revokeObjectURL(objUrl), 1000);
                    } catch (e) {
                      console.error('[handoff-pdf] error', e);
                      alert(`PDF download failed: ${e instanceof Error ? e.message : 'unknown'}`);
                    }
                  }}
                  className="flex w-full items-center justify-center gap-2 rounded-lg border border-dash-border bg-dash-card px-4 py-2.5 text-xs font-semibold text-text-secondary transition-colors hover:border-sc-700 hover:text-sc-400"
                >
